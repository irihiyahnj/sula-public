...
- Git upgrade command examples now read `site/sula.json` from a fresh shallow Git clone of `sula-public` main, avoiding HTTP raw endpoint caching entirely.
- Model-facing upgrade prompts point to the GitHub blob view of `site/sula.json` while preserving the rule that agents must upgrade from the immutable `source_ref` tag.

### Sync Impact

- No managed project files change beyond normal version metadata.
- External models can discover the current upgrade tag through Git itself, then use that tag as the fixed release baseline.

## 0.18.3 - 2026-05-06

### Changed

- Git upgrade docs now use the GitHub `raw/refs/heads/main` descriptor URL, avoiding stale CDN reads observed from `raw.githubusercontent.com/.../main`.

### Sync Impact

- No managed project files change beyond normal version metadata.
- External models should read `https://github.com/irihiyahnj/sula-public/raw/refs/heads/main/site/sula.json`, then upgrade from the immutable tag named by `source_ref`.

## 0.18.2 - 2026-05-06

### Changed

- Git upgrade runbook and model-facing upgrade prompts now resolve the current `source_ref` from the public GitHub descriptor at `raw.githubusercontent.com/irihiyahnj/sula-public/main/site/sula.json`.
- Upgrade instructions no longer depend on the older hosted `sula.1stp.monster` descriptor, which may lag the Git-backed public source.

### Sync Impact

- No managed project files change beyond normal version metadata.
- External models should use the GitHub descriptor as the live update channel, then use the descriptor's tagged `source_ref` as the immutable upgrade baseline.

## 0.18.1 - 2026-05-06

### Changed

- Git release upgrade docs now resolve the current published tag from `site/sula.json` / `source_ref` instead of hard-coding a duplicate stable-version label in model-facing instructions.
- Model upgrade prompts now tell external coding agents to read the launch contract and machine-readable descriptor before selecting the Git tag to use.
- README upgrade examples now use the published descriptor as the version source of truth.

### Sync Impact

- No managed project files change beyond normal version metadata.
- Operators can now give other models the Sula upgrade prompt without manually updating embedded version numbers after each release.

## 0.18.0 - 2026-05-06

### Added

- `agent-routing configure` command that lets an operator choose the executor runner, local CLI command, provider, model, workspace mode, and write-access policy once, then reuse that remembered route until `--replace` or explicit new values are provided.
- Machine-local provider registration for remembered executor routes under `.sula/local/agent-providers.json`, preserving the no-secret rule by storing provider metadata and env var names rather than raw credentials.
- Tests for remembered executor routing, route reuse, and explicit replacement.

### Changed

- `site/sula.json`, `site/launch/bootstrap.py`, `.sula/version.lock`, and `VERSION` now identify the `v0.18.0` source baseline.
- Agent routing docs now describe the first-run configure flow for Claude Code, Hermes, DeepSeek-backed wrappers, and other local executor commands.

### Sync Impact

- Existing projects keep their current routing until they opt in.
- Projects that want external executor routing should run `python3 scripts/sula.py agent-routing configure --project-root .` once after syncing.
- Sula Core still does not vendor DeepSeek, OpenAI, Claude, or Hermes SDKs; provider API mechanics remain in local runners or CLI wrappers.

## 0.17.0 - 2026-05-05

### Added

- Optional dependency-light `mcp` command family with `tools`, `call`, and stdio `serve` entrypoints for a Sula MCP-compatible control surface.
- Read-only MCP tools for project bootstrap, status, check, doctor, query, memory digest, rules, artifacts, workflow assessment, orchestration tasks/runs, portfolio list/status, and provider capabilities.
- Controlled MCP write tools for reports, workflow scaffolds, orchestration intake/closeout, artifact registration/materialization/refresh, and sync dry-run, with explicit write-class enablement and audit events.
- Consolidated project policy view that exposes highest rule, workflow policy, agent behavior, orchestration approval categories, verification adapters, software constraints, service constraints, and instruction-file summaries.
- Provider capability report for local filesystem and Google Drive style providers.
- Structured PR closeout evidence for CI state, review action counts, unresolved review threads, and closeout state.
- Non-software service canary coverage for the agent-native control surface.

### Changed

- `site/sula.json` and `VERSION` now identify the `v0.17.0` source baseline.
- Portfolio status now has a high-signal MCP payload for blockers, open tasks, pending runs, stale memory, and stale artifact risks.
- The Sula overview and README now point at the 0.17.0 agent-native project OS whitepaper and execution package.

### Sync Impact

- Existing CLI workflows remain compatible; MCP is an optional control surface, not a replacement for `scripts/sula.py`.
- Read-only MCP calls do not require a local policy file. Controlled write tools require an explicit project root allowlist and enabled write class.
- No provider-native writes, arbitrary shell execution, release publication, or destructive operations are enabled by default.
