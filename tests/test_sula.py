...
""",
    ...
    def file_snapshot(self, project_root: Path) -> dict[str, str]:
...
""",
    ...
    def test_site_descriptor_points_to_published_public_repo(self) -> None:
        descriptor = json.loads((REPO_ROOT / "site" / "sula.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(descriptor["source_ref"], "v0.18.2")
    ...
    def test_mcp_readonly_bootstrap_and_rules_do_not_write_project_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(adopt_result.returncode, 0, adopt_result.stderr)
            before = self.file_snapshot(project_root)
            ...
            self.assertTrue(payload["rules"]["service_constraints"]["applies"])
            self.assertIn("provider_capabilities", payload)
            self.assertEqual(before, self.file_snapshot(project_root))

    def test_mcp_controlled_report_requires_policy_and_writes_audit(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            self.create_generic_project(project_root)
            adopt_result = run_cli("adopt", "--project-root", str(project_root), "--approve")
            ...
            blocked = run_cli(
                ...
                str(project_root),
                "--arguments-json",
                json.dumps({"summary": "MCP report should be blocked without write policy."}),
            )
            ...
            allowed = run_cli(
                ...
                "record",
                "--arguments-json",
                json.dumps({"summary": "MCP report created through Sula.", "lightweight": True}),
            )

            self.assertEqual(allowed.returncode, 0, allowed.stderr)
            payload = json.loads(allowed.stdout)
            self.assertEqual(payload["status"], "ok")
            self.assertEqual(payload["mcp_audit"]["event_type"], "mcp.write")
            self.assertIn("MCP report created through Sula.", (project_root / "STATUS.md").read_text(encoding="utf-8"))
            self.assertIn('"tool": "sula.report.create"', (project_root / ".sula" / "events" / "log.jsonl").read_text(encoding="utf-8"))

    def test_mcp_portfolio_status_and_provider_capabilities_cover_service_projects(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir, tempfile.TemporaryDirectory() as portfolio_tmpdir:
            ...
            self.create_generic_project(project_root)
            adopt_result = run_cli("adopt", "--project-root", str(project_root), "--storage-provider", "google-drive", "--approve")
            self.assertEqual(adopt_result.returncode, 0, adopt_result.stderr)
            register_result = run_cli("portfolio", "register", "--project-root", str(project_root), "--portfolio-root", str(portfolio_root), "--json")
            ...
            self.assertEqual(portfolio_payload["totals"]["projects"], 1)
            self.assertEqual(portfolio_payload["projects"][0]["storage_provider"], "google-drive")

            provider_result = run_cli(
                "mcp",
                "call",
                "--tool",
                "sula.provider.capabilities",
                "--project-root",
                str(project_root),
                "--arguments-json",
                "{}",
            ...
            self.assertEqual(provider_payload["capability_report"]["provider"], "google-drive")
            self.assertIn("fetch_item", provider_payload["capability_report"]["capabilities"])

    def test_mcp_stdio_serves_tools_list(self) -> None:
        request_payload = {"jsonrpc": "2.0", "id": 1, "method": "tools/list", "params": {}}
        result = run_cli_input(json.dumps(request_payload) + "\n", "mcp", "serve")

        self.assertEqual(result.returncode, 0, result.stderr)
        response = json.loads(result.stdout.strip().splitlines()[-1])
        self.assertEqual(response["id"], 1)
        tool_names = [item["name"] for item in response["result"]["tools"]]
        self.assertIn("sula.project.bootstrap", tool_names)
        self.assertIn("sula.report.create", tool_names)
    ...
    def test_session_start_surfaces_visible_execution_and_unknown_host_model(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            self.create_generic_project(project_root)
            ...
            self.assertEqual(payload["command"], "session.start")
            self.assertEqual(payload["memory_digest"], ".sula/memory-digest.md")
            self.assertEqual(payload["host"]["model"], "unknown")
            self.assertIn("agent_routing", payload)
            ...
            self.assertEqual(explicit_payload["host"]["agent"], "codex-cli")
    ...
    def test_agent_routing_provider_doctor_is_local_and_secret_safe(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            adopt_result = run_cli("adopt", "--project-root", str(project_root), "--approve")
            self.assertEqual(adopt_result.returncode, 0, adopt_result.stderr)
            local_dir = project_root / ".sula" / "local"
            local_dir.mkdir(parents=True, exist_ok=True)
            (local_dir / "agent-providers.json").write_text(
                json.dumps(
                    {
                        "providers": {
                            "deepseek": {
            ...
            manifest_text = manifest_text.replace('model = "configured-runner"', 'model = "deepseek-flash"', 1)
            manifest_path.write_text(manifest_text, encoding="utf-8")
            ...
            status_result = run_cli(
                ...
                str(project_root),
                "--json",
                env={"SULA_DEEPSEEK_API_KEY": "test-secret-value"},
            )
            self.assertEqual(status_result.returncode, 0, status_result.stderr)
            self.assertNotIn("test-secret-value", status_result.stdout)
            payload = json.loads(status_result.stdout)
            executor = payload["agent_routing"]["provider_readiness"]["roles"]["executor"]
            self.assertEqual(executor["provider"], "deepseek")
            self.assertTrue(executor["ready"])

            bad_config = json.loads((local_dir / "agent-providers.json").read_text(encoding="utf-8"))
            bad_config["providers"]["deepseek"]["api_key"] = "raw-secret"
            (local_dir / "agent-providers.json").write_text(json.dumps(bad_config), encoding="utf-8")
            doctor_result = run_cli("agent-routing", "doctor", "--project-root", str(project_root), "--json")
            self.assertEqual(doctor_result.returncode, 1)
            self.assertNotIn("raw-secret", doctor_result.stdout)
            doctor_payload = json.loads(doctor_result.stdout)
            self.assertTrue(any("must not store raw `api_key`" in item for item in doctor_payload["errors"]))

    def test_agent_routing_configure_remembers_executor_choice_until_replaced(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            self.create_generic_project(project_root)
            ...
            replace_payload = json.loads(replace_result.stdout)
            self.assertEqual(replace_payload["routing"]["runner"]["command"], "hermes run")
            self.assertEqual(replace_payload["routing"]["executor"]["provider"], "hermes")
            self.assertEqual(replace_payload["routing"]["executor"]["model"], "deepseek-v4")

    def test_role_aware_runner_request_and_visible_active_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            self.create_generic_project(project_root)
            runner_script = project_root / "role_runner.py"
            request_path = project_root / "runner-request.json"
            runner_script.write_text(
                "\n".join(
                    [
                        ...
                        f"Path({str(request_path)!r}).write_text(json.dumps(payload, indent=2), encoding='utf-8')",
                        "print(json.dumps({",
                        "  'status': 'human-review',",
                        "  'summary': 'role aware runner completed',",
                        "  'validation_evidence': [{'kind': 'role-aware-check', 'summary': 'verification passed'}]",
                        "}))",
                    ]
                ),
                encoding="utf-8",
            ...
            intake_result = run_cli(
                ...
                "--acceptance",
                "routing request includes executor model",
                "--validation",
                "verification passed",
                "--json",
            )
            ...
            self.assertEqual(run_result.returncode, 0, run_result.stderr)
            run_payload = json.loads(run_result.stdout)
            self.assertEqual(run_payload["run"]["agent_routing"]["roles"]["executor"]["provider"], "deepseek")
            ...
            self.assertEqual(status_result.returncode, 0, status_result.stderr)
            status_payload = json.loads(status_result.stdout)
            active = status_payload["orchestration"]["active"]["active"]
            self.assertEqual(active["role"], "reviewer")
            self.assertIn(active["state"], ["human-review", "accepted"])
            timeline_result = run_cli("orchestration", "timeline", "--project-root", str(project_root), "--json")
            self.assertEqual(timeline_result.returncode, 0, timeline_result.stderr)
            timeline_payload = json.loads(timeline_result.stdout)
            roles = [item["role"] for item in timeline_payload["events"]]
    ...
    def test_orchestration_closeout_requires_remote_pr_verification_when_configured(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir, tempfile.TemporaryDirectory() as fixture_tmpdir:
                ...
                json.dumps(
                    {
                        ...
                        "checks_summary": {"state": "passed", "total": 3, "passed": 3, "failed": 0, "pending": 0},
                        "review_summary": {"unresolved_threads_count": 0, "comments_requiring_action_count": 0},
                    },
                    indent=2,
                )
                + "\n",
            ...
            pr_check = next(item for item in checks if item.get("kind") == "pull-request-url")
            self.assertEqual(pr_check["ci_state"], "passed")
            self.assertEqual(pr_check["unresolved_review_threads_count"], 0)
            self.assertEqual(pr_check["closeout_state"], "merged")
...
""",
            ...
            self.assertEqual(
                [item["name"] for item in payload["reports"][0]["checks"]],
                ["sync_dry_run", "doctor_strict", "memory_digest", "check"],
            )
    ...
    def test_check_fails_when_recent_decisions_exceed_current_state_limit(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            count = 101
            oversized_decisions = "\n".join(
                [f"- 2026-04-{day:02d}: decision {index}" for index, day in enumerate(range(11, 11 + count), start=1)]
            )
            status_text = status_text.replace(
                "- 2026-04-11: established the initial memory contract",
                oversized_decisions,
            ...
            failed = run_cli("check", "--project-root", str(project_root))
            self.assertNotEqual(failed.returncode, 0)
            self.assertIn(f"`## Recent Decisions` has {count} items", failed.stdout)
    ...
    def test_check_fails_when_current_focus_and_blockers_exceed_limits(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            status_text = status_path.read_text(encoding="utf-8")
            count = 101
            oversized_focus = "\n".join([f"- focus item {index}" for index in range(1, count + 1)])
            oversized_blockers = "\n".join([f"- blocker item {index}" for index in range(1, count + 1)])
            ...
            failed = run_cli("check", "--project-root", str(project_root))
            self.assertNotEqual(failed.returncode, 0)
            self.assertIn(f"`## Current Focus` has {count} items", failed.stdout)
            self.assertIn(f"`## Blockers` has {count} items", failed.stdout)
    ...
    def test_memory_digest_archives_current_focus_and_blocker_overflow(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(len(focus_lines), 100)
            self.assertEqual(len(blocker_lines), 100)
    ...
    def test_record_new_trims_recent_decisions_to_current_state_limit(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            count = 105
            prefill = "\n".join(
                [f"- 2026-04-{day:02d}: pre-existing decision {index}" for index, day in enumerate(range(1, count + 1), start=1)]
            )
            status_text = status_text.replace(
                "- 2026-04-11: established the initial memory contract",
                prefill,
            ...
            self.assertEqual(len(recent_decision_lines), 100)
            self.assertTrue(any("2026-04-30" in line for line in recent_decision_lines))
            archive_text = (project_root / "docs" / "ops" / "status-archive.md").read_text(encoding="utf-8")
            self.assertIn("pre-existing decision 1", archive_text)
            self.assertIn("pre-existing decision 2", archive_text)
