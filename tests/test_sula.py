...
""",
    ...
    def test_site_descriptor_points_to_published_public_repo(self) -> None:
        ...
        self.assertEqual(descriptor["source_ref"], "v0.14.0")
    ...
    def test_orchestration_defaults_to_disabled_and_supports_dry_run_records(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            self.create_generic_project(project_root)
            ...
            self.assertIn("enabled = false", manifest_text)
            ...
            self.assertIn("orchestration is disabled in the manifest", blocked_payload["run"]["blocked_reasons"])

            manifest_path.write_text(manifest_text.replace("enabled = false", "enabled = true"), encoding="utf-8")
            dry_run = run_cli(
                "orchestration",
                "run",
                "--project-root",
    ...
    def test_orchestration_trigger_and_shell_command_runner_collect_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manifest_path = project_root / ".sula" / "project.toml"
            manifest_text = manifest_path.read_text(encoding="utf-8")
            manifest_text = manifest_text.replace("enabled = false", "enabled = true")
    ...
    def test_automation_execute_mode_dispatches_low_risk_intent(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            project_root = Path(tmpdir)
            self.create_generic_project(project_root)
            ...
            manifest_path = project_root / ".sula" / "project.toml"
            manifest_text = manifest_path.read_text(encoding="utf-8")
            manifest_text = manifest_text.replace("enabled = true\nmode = \"assist\"", "enabled = true\nmode = \"execute\"")
            manifest_text = manifest_text.replace("auto_dispatch = false", "auto_dispatch = true")
            manifest_text = manifest_text.replace("[orchestration]\nenabled = false", "[orchestration]\nenabled = true")
            manifest_path.write_text(manifest_text, encoding="utf-8")
