...
- local executor wrappers should follow [docs/reference/local-executor-wrapper-contract.md](docs/reference/local-executor-wrapper-contract.md): consume `SULA_EXECUTOR_CONTRACT_JSON` and `SULA_EXECUTION_PACKET_JSON`, call the local model CLI non-interactively, and return one Sula JSON object with status, summary, touched files, validation evidence, and usage/cost metrics
