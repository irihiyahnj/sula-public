...
- machine-readable descriptor: `https://raw.githubusercontent.com/irihiyahnj/sula-public/main/site/sula.json`
...
```text
请按 Sula 发布升级协议处理当前项目：先读取 https://raw.githubusercontent.com/irihiyahnj/sula-public/main/site/sula.json ，取得当前发布源和 source_ref；再使用该 source_ref 对应的 Sula 公开 Git tag 作为唯一升级基线，对当前仓库执行完整升级。你需要先检查旧文件和现有 .sula 状态，识别所有与最新稳定版不一致的内容，再完成 sync、memory digest、doctor --strict、check，并在结束时报告变更、遗留风险、是否已达到稳定版要求，以及后续使用方式。
```

### 2. Strict Audit Upgrade

```text
请按 Sula 严格升级审计协议处理当前项目：先读取 https://raw.githubusercontent.com/irihiyahnj/sula-public/main/site/sula.json ，取得当前发布源和 source_ref；再基于该 source_ref 对应的 Sula 公开 Git tag 对当前仓库进行一次完整的升级审计。你必须遍历旧的 managed files、docs、.sula 状态和 release/check 相关文件，找出所有不符合最新稳定版规范的地方；然后执行升级、修复必要问题、重建派生状态，并最终确认 doctor --strict 和 check 是否通过，哪些地方已达标，哪些地方仍需人工决策。
```

### 3. Batch Fleet Upgrade

```text
请按 Sula 批量升级协议处理这一批项目：先读取 https://raw.githubusercontent.com/irihiyahnj/sula-public/main/site/sula.json ，取得当前发布源和 source_ref；并以该 source_ref 对应的 Sula 公开 Git tag 作为唯一升级基线。对每个项目都先检查旧文件、现有 .sula 状态和与稳定版规范的差异，再执行完整升级与验证流程；每个项目最后都要输出升级结果、失败点、遗留风险、是否达到稳定版要求，以及是否还需要继续处理 staged memory 或旧 release 文档问题。
```
...
```text
请读取 https://raw.githubusercontent.com/irihiyahnj/sula-public/main/site/sula.json ，取得当前发布源和 source_ref，并基于该 source_ref 对应的 Sula 公开 Git tag，把当前项目完整升级到最新稳定规范，遍历旧文件与 .sula 状态，修复不一致项，直到 doctor --strict 和 check 达到稳定版要求，再汇报结果。
```
