...
### `[orchestration]` (optional; enabled by default with dry-run)

- `enabled`: defaults to `true` so task listing, dry-run scheduling, closeout records, and automation-planned work are available immediately
...
### `[automation]` (optional; execute/dry-run by default)

- `enabled`: controls whether Sula records command/provider/status events into the automation state inbox
- `mode`: `observe`, `assist`, or `execute`; `execute` is the default so eligible low-risk intents are dispatched automatically into the configured runner
...
- `auto_dispatch`: defaults to `true`; allows `execute` mode to start eligible runs automatically
...
### MCP local policy (machine-local, not manifest truth)

Sula 0.17.0's MCP-compatible surface is configured locally, not through `.sula/project.toml`.

Use `.sula/local/mcp-policy.json` or equivalent command-line flags for the current machine:

```json
{
  "allowed_project_roots": ["/path/to/project"],
  "enabled_write_classes": ["record", "generate"]
}
```

- Read-only MCP tools can be called with an explicit `--project-root`.
- Controlled write tools require an allowlisted project root and an enabled write class.
- This file is local access policy, not project-owned business truth and not a centrally managed template.
- Strong operations such as provider-native writes, release publication, destructive actions, and runner execution are not enabled by default.
...
### `[agent_routing]` (optional but recommended)

- `enabled`
- `mode`: `off`, `assist`, `plan-execute-review`, `review-only`, or `executor-only`
...
- `[agent_routing.roles.<role>]`: role configuration for `host`, `planner`, `executor`, `verifier`, `reviewer`, and `acceptor`
...
- The optional `[orchestration]` section is Sula's Symphony-style control-plane contract. It is enabled by default with local task files and the non-mutating `dry-run` runner, and records state under `.sula/state/orchestration/` rather than turning runner prompts or tracker metadata into project truth.
...
- The optional `[automation]` section is the event-driven Sula kernel above orchestration. Sula commands such as `check`, `doctor`, `status`, `query`, `sync`, and artifact freshness operations can automatically record events under `.sula/state/automation/`, classify failed or freshness-related events into intents, and dispatch eligible low-risk intents into orchestration. Manual `orchestration trigger` remains available for backfill and debugging, but normal Sula usage should not depend on users remembering to trigger work by hand.
- MCP local policy intentionally stays outside `.sula/project.toml`: project manifests describe durable project behavior, while `.sula/local/mcp-policy.json` describes what the current machine allows an agent connector to do.
- The default runner remains `dry-run`, so default automatic dispatch creates auditable run records and user-visible feedback without mutating the project. Real mutation requires an explicitly configured real runner plus passing risk and approval gates.
...
- The optional `[agent_routing]` section records role-based model routing preferences. Sula can show planner/executor/verifier/reviewer model assignments in any CLI and can pass role/model hints to runner adapters, but the current host chat model remains an observed session fact unless the CLI or user provides metadata.
- `.sula/local/agent-providers.json` is local access policy, not project truth. It may name `endpoint_env` and `api_key_env` variables for OpenAI-compatible providers such as DeepSeek, but it must not contain raw credentials.
...
- The `[projection]` section separates Sula's kernel capabilities from the repo-visible governance surface. New adoptions default to `governed`, so the full visible operating surface is available immediately unless a project intentionally reduces it.
